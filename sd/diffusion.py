...
class UNET_ResidualBlock(nn.Module):
    ...
    def forward(self, feature, time):
        ...
        # merged = feature (Batch_size, In_channels, Height, Width) + time (1, 1280, 1, 1)
...
class UNET_AttentionBlock(nn.Module):
    def __init__(self, n_head: int, n_embed: int, d_context=788):
        super().__init__()
        channels = n_head * n_embed
        
        self.groupnorm = nn.GroupNorm(32, channels, 1e-6)
    ...
    def forward(self, x, context):
        ...
        self.attention_2(x, context) # Cross Attention
...
class SwitchSequential(nn.Sequential):
    def forward(self, x: torch.Tensor, context: torch.Tensor, time:torch.Tensor) -> torch.Tensor:
        for layer in self:
            if isinstance(x, UNET_AttentionBlock):
                x = layer(x, context)
...
#             # (Batch_Size, 320, Height / 32, Width / 32) -> (Batch_Size, 320, Height / 64, Width / 64)
